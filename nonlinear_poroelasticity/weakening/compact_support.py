...
times = np.linspace(0.0, N_time * delta_t, N_time + 1)
...
v_s = t_v_s * (vt / t_v +
               phi_f.g * k_e.g * (E.g * g.g).dx(0) / ((1 - b - a) * (1 - phi_f.g) * t_phi))
v_f = t_v_f * (vt / t_v - k_e.g * (E.g * g.g).dx(0) / ((1 - b - a) * t_phi))
# v_s = t_v_s * (vt / t_v +
...
Fun_phi = ((dphi_dt + (- db_dt - da_dt) * phi_f.g / (1 - b - a)) * phi_f.v / t_sc * dx +
           ((1 / (1 - b - a))**2 * phi_f.g * k_e.g * (E.g * g.g).dx(0) / t_phi -
            (1 / (1 - b - a)) * phi_f.g * (vt / t_v - ((1 - xi) * da_dt - xi * db_dt) / t_sc)) * phi_f.v.dx(0) * dx +
           (vt / t_v - ((1 - xi) * da_dt) / t_sc) * phi_f.v / (1 - b - a) * ds)

# Fun_sigma = (sigma_l - (E.g * g.g) / (1 - phi_f0)) * (1 - xi) * sigma.v * ds

# Weak form for the E equation
Fun_E = (dE_dt / t_sc + c.g * (E.g - E_min) / t_E
         + (v_s / t_v_s - ((1 - xi) * da_dt - xi * db_dt) / t_sc) / (1 - b - a) * E.g.dx(0)) * E.v * dx
# Fun_E = dE_dt * E.v / t_sc * dx + c.g * (E.g - E_min) * E.v / t_E * dx

# Weak form for the c equation
Fun_c = ((phi_f.g * dc_dt + dphi_dt * c.g +
          (- db_dt - da_dt) * c.g * phi_f.g / (1 - b - a)) / t_sc * c.v * dx +
         phi_f.g / (1 - b - a) *
         (c.g.dx(0) / ((1 - b - a) * t_c) -
...
Fun_us = ((u_s.g.dx(0) * u_s.v -
           (phi_f.g - phi_f0) * (1 - b - a) / (1 - phi_f0) * u_s.v) * dx)
#
# Weak form for the left moving boundary (both forms below are valid)
Fun_a = (phi_f.g - 1 + (1 - b - phi_f0) / (1 - b - a)) * v_a * dx
...
"""
# t_0 is time of compact support. This default value ensures that if no t_0 arises,
# then we skip the compact support loop
t_0 = (N_time + 1) * delta_t
v_list = [float(vt(0.0))]
phi_min = 0.0
for n in range(N_time):
    ...
    if phi_f.f_fixed[-1] < 0.0:
        print(phi_f.f_fixed[-1])
        phi_f.f_fixed[-1] = phi_min
    ...
    bcs[-1] = bc_left_phi
    if phi_r <= phi_min:
        print("Porosity on the right has reached zero, finishing initial problem...")
        t_0 = n * delta_t
        break

bc_right_phi = DirichletBC(V.sub(0), phi_min, right)
bcs.append(bc_right_phi)

b_guess = Expression("0", degree=1, t=t_0, t_0=t_0)
# Fun_b_compact_support = (xi * (1 - b - a + t_v * phi_f.g * k_e.g * (E.g * g.g).dx(0) /
#                                (vt * (1 - phi_f.g) * t_phi))) * v_b * ds
Fun_b_compact_support = (xi * v_s) * v_b * ds
# Fun_b_compact_support = (b - b_guess) * v_b * ds
# Fun_b_compact_support = (xi * ((1 - b - a) / (E.g * g.g).dx(0) +
#                                (t_v * phi_f.g * k_e.g) / (vt * t_phi * (1 - phi_f.g)))) * v_b * ds
Fun = Fun_phi + Fun_E + Fun_c + Fun_sigma + Fun_us + Fun_a + Fun_b_compact_support

print("Starting compact support problem...")
...
for n in range(int(t_0 / delta_t), N_time):
    ...
    # sprms['relaxation_parameter'] = 0.5
    # sprms['relative_tolerance'] = 0.4
    # sprms['absolute_tolerance'] = 0.007
    #
    ...
    (phi_f.f_fixed, E.f_fixed, c.f_fixed, sigma.f_fixed,
     u_s.f_fixed, v_s_.f_fixed) = xi_t_to_x_t(mesh, a_list[-1], b_list[-1], phi_f, E,
                                              c, sigma, u_s, v_s_)
    # plot at the current timepoint if needed
    _, phiarr = fenics_to_numpy(mesh, phi_f.f)
    # print(phiarr[-10:])
    if phi_f.f_fixed[-1] < phi_min:
        phi_f.f_fixed[-1] = phi_min
    phi_r = phi_f.f_fixed[-1]
    if (n + 1) % plotting_freq == 0:
        Quantity.plot_quantities(quantities, norm, (n + 1) * delta_t, saving,
...
times = np.linspace(0.0, len(a_list) * delta_t, len(a_list))
