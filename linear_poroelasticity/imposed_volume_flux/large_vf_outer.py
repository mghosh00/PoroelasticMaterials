...
sigma_xx0_prime = Quantity("$\\sigma_{xx,0}'$", "YlOrBr",
...
_, phi_array = get_phi(mesh, sigma_xx0_prime.f, M_0.f, phi_f0, epsilon)
x_coords, u_array = get_u(mesh, phi_array, phi_f0, epsilon)

phi_f1 = Quantity("$\\epsilon\\phi_{f,1}$", "Blues", 3, mesh)
...
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=c_0.cmap),
             ...
             label='$t$', ax=c_0.ax)
c_0.label_plot("Leading order solute concentration", label_size='xx-large')

# M_0
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=M_0.cmap),
             orientation='vertical',
             label='$t$', ax=M_0.ax)
M_0.label_plot("Leading order p-wave modulus", label_size='xx-large')

# sigma_xx,0'
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=sigma_xx0_prime.cmap),
             orientation='vertical',
             label='$t$', ax=sigma_xx0_prime.ax)
sigma_xx0_prime.label_plot("Leading order Terzaghi stress", label_size='xx-large')
sigma_xx0_prime.ax.set_ylim(-2.5, 0.5)
...
phi_f1.label_plot("First order porosity", label_size='xx-large')
...
fig.savefig(f"plots/Q_step_oop_new/fenics_b_M_{b_M_num}_Pe_{Pe_num}.png", bbox_inches="tight")
