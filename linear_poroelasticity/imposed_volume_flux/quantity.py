...
class Quantity:
    ...
    def plot(self, norm: mpl.colors.Normalize, time: float,
             save_data: bool = False, file_name: str = '', alpha: float = 1.0,
             x_arr: np.array = None):
        ...
        :param alpha: The alpha for the plot.
        :param x_arr: An optional x-range for the plot, if it is not the same as the
                      mesh.
        """
        mesh_array, f_array = self.fenics_to_numpy(self._mesh, self.f)
        if self.ax:
            if x_arr is not None:
                self.ax.plot(x_arr, f_array, color=self.cmap(norm(time)),
                             alpha=alpha)
            else:
    ...
    def plot_quantities(quantities, norm: mpl.colors.Normalize,
                        ...
                        file_names: list[str] = None, alpha: float = 1.0,
                        x_arr: np.array = None):
        ...
        if save_list:
            for i, quantity in enumerate(quantities):
                quantity.plot(norm, time, save_list[i], file_names[i], alpha, x_arr)
        else:
            for i, quantity in enumerate(quantities):
                quantity.plot(norm, time, alpha=alpha, x_arr=x_arr)
